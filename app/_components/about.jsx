...
export default function About() {
    ...
    return (
        <div className="h-fit py-16 px-8 md:px-24 bg-gray-100 text-gray-900">
            <div className="grid md:grid-cols-3 place-items-center gap-8">
                <img ref={ref} src="/raya.png" alt="Raya Band Photo" className="hover:scale-110 duration-1000 h-auto md:h-80 rounded-full col-span-3 md:col-span-1" />
                <div className='space-y-4 col-span-2'>
                    <h2 className="text-2xl md:text-4xl font-bold mb-4 md:mb-8">Acerca de nosotros</h2>
                    <div className='space-y-4'>
                        <p className="md:text-lg leading-relaxed">
                            {paragraphOne.map((word, index) => (
                                <span
                                    key={index}
                                    className={`inline-block transform transition-transform duration-500 ${inView || isMatch ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'}`}
                        ...
                        <p className="md:text-lg leading-relaxed">
                            {paragraphThree.map((word, index) => (
                                <span
                                    key={index}
                                    className={`inline-block transform transition-transform duration-500 ${inView || isMatch ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}
                                    style={{ transitionDelay: `${index * Math.random() * 10}ms` }}
                                >
                                    {word}&nbsp;
                                </span>
